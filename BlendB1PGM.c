...
void blend()
/* マスク画像を作成する */
/* 背景画像gimage_work[0][y][x]と注目画像gimage_work[1][y][x]の輝度差に基づきマスク画像作成 */
{
	int x, y;
	...
	/* ラスタ走査で各画素の輝度の差を計算して閾値と比較し，マスク画像をgimage_out[y][x]に格納 */
	for ( y = 0; y < y_size2; y ++ ) {
		for ( x = 0; x < x_size2; x ++ ) {
			/* 注目画像と背景画像の輝度差の絶対値を計算しdifに格納 */
			int dif = gimage_work[1][y][x] - gimage_work[0][y][x];
			if (dif < 0) dif *= -1;
		    /* ①　差の絶対値difと閾値tlvを比較 */
		    /* 　　閾値より大きい→ 前景，　閾値以下→背景 */
		    /* 　　前景ならば白（255），背景ならば黒（0）をgimage_out[y][x]に代入 */
			/* 　　プログラム作成 */
			gimage_out[y][x] = dif > tlv ? 255 : 0;
		}
	}
}
